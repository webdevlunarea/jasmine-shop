    ...
    <div class="bg-white p-4 rounded" style="box-shadow: 0 0 10px rgba(0,0,0,0.4);">
        <div class="d-flex justify-content-between align-items-start">
            ...
            <div class="d-flex gap-2">
                <a class="btn d-flex" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Simpan"
                    onclick="acteditresi()"><i class="material-icons" style="font-size:large">check</i></a>
                <a class="btn d-flex" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Batal"
                    onclick="closeeditresi()"><i class="material-icons" style="font-size:large">close</i></a>
            </div>
        </div>
                ...
                <div class="d-flex">
                    ...
                    <div style="gap: 3px; width: 100px;">
                        <a class="btn btn-success d-flex <?php switch ($t['status']) {
                            ...
                            data-bs-title="Edit resi"
                            onclick="editresi('<?= $t['id_midtrans']; ?>', '<?= $t_ind; ?>')"><i class="material-icons"
                                style="font-size:large">mode_edit</i></a>
                        <a class="btn btn-success d-flex" data-bs-toggle="tooltip" data-bs-placement="top"
                            data-bs-title="Download" href="/pdf/<?= $t['id_midtrans']; ?>"><i class="material-icons"
                                style="font-size:large">file_download</i></a>
                    </div>
                </div>
                    ...
                    <div class="w-100 pd-2">
                        ...
                        <div class="d-flex flex-column w-100 mb-2">
                            ...
                            <div class="d-flex w-100 align-items-start">
                                ...
                                <p style="flex: 1;" class="mb-0 text-end fw-bold">Rp
                                    <?= number_format((json_decode($t['data_mid'], true)['gross_amount'] - $totalHargaBarang), 0, ",", "."); ?>
                                </p>
                            </div>
                            <div class="d-flex w-100 align-items-start">
                                <p style="flex: 3;" class="mb-0">Total Keseluruhan</p>
                                <p style="flex: 1;" class="mb-0 text-end fw-bold">Rp
                                    <?= number_format(json_decode($t['data_mid'], true)['gross_amount'], 0, ",", "."); ?>
                                </p>
                            </div>
                        </div>
                        <?php if ($t['status'] == "Menunggu Pembayaran") { ?>
                        <p class="mb-0">
                            <b><?= ucfirst(str_replace('_', ' ', json_decode($t['data_mid'], true)['payment_type'])); ?></b>
                        </p>
                        <p class="mb-0">
                            <?= json_decode($t['data_mid'], true)['payment_type'] == "bank_transfer" ? strtoupper(json_decode($t['data_mid'], true)['va_numbers'][0]['bank']) . " " . json_decode($t['data_mid'], true)['va_numbers'][0]['va_number'] : "" ?>
                        </p>
                        <p class="mb-0">
                            <?= json_decode($t['data_mid'], true)['payment_type'] == "echannel" ? "Biller Code: " . json_decode($t['data_mid'], true)['biller_code'] . "<br>Bill Key: " . json_decode($t['data_mid'], true)['bill_key'] : "" ?>
                        </p>
                        <?php } ?>
